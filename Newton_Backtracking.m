...
h = [exp(-x(1)-x(2)+1)+exp(x(1)+x(2)-1)+2 exp(1-x(1)-x(2))+exp(x(1)+x(2)-1)+1; 
     exp(1-x(1)-x(2))+exp(x(1)+x(2)-1)+1 exp(-x(1)-x(2)+1)+exp(x(1)+x(2)-1)+2];
end

function d = direction(h,g)
d = -(transpose(h)*g);
end
